...
function App() {
  ...
  const AcuityFuture = () => {
    if (futureAp.length !== 0) {
      return (
        <>
          <h2 id="toShow0" className="tb-header">
            ACUITY Future Appointments
          </h2>
          <div id="toShow01" className="tb-table-container">
            <Table
              tableName={"Acuity"}
              columns={columnsAcuity}
              data={futureAp}
            />
          </div>
        </>
      ...
      return (
        <h2 id="toShow00" className="tb-header">
  ...
  const AcuityPast = () => {
    ...
    if (pastAp.length !== 0) {
      return (
        <>
          <h2 id="toShow" className="tb-header">
            ACUITY Past Appointments
          </h2>
  ...
  const Airtable = () => {
    if (data.Textline.length !== 0) {
      return (
        <>
          <h2 id="toShow4" className="tb-header">
            AIR-TABLE{" "}
          </h2>
    ...
    <div>
      ...
      {err && (
        <Alert sx={{ width: "70%", margin: "auto" }} severity="info">
          ...
          Sorry, it seems this contact has no entries at Airtable, Acuity &
          Textline
        </Alert>
      )}
      {isTableDataLoading ? (
...
const Header = ({
  ...
  const fetchData = async () => {
    // debugger
    let arr = [];
    const data = await fetch("https://web-tables-api.herokuapp.com/textline");
    const data2 = await fetch("https://web-tables-api.herokuapp.com/acuity");
    const data3 = await fetch("https://web-tables-api.herokuapp.com/airtable");
  ...
  function formatDate(string) {
    let date =
      new Date(string.substring(0, 10)).toLocaleDateString("en-US") + " ";
    let time = new Date(
      string.substring(0, 10) + " " + string.substring(11, 18)
  ...
  const handleSubmitClick = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://hook.us1.make.com/der2itrt38fuk2rsmpclth7qohnaivj1?phone=${phone1}&email=${email}`
      );
      ...
      response.json().then((value) => {
        // debugger
        if (
          value.Airtable[0]["First Name"] === undefined &&
          value.Acuity[0]["First Name"] === undefined
        ) {
          setErr(true);
          hideTables();
          return;
        ...
        for (var i = 0; i < len; i++) {
          if (i < Textlinelength) {
            value.Textline[i].Timestamp = formatDate(
              value.Textline[i].Timestamp
            );
          }
          if (i < Acuitylength) {
        ...
        value.Acuity.map((v, index) => {
          ...
          if (date.getTime() < todayDate.getTime()) pastArray.push(v);
          else futureArray.push(v);
    ...
    <div>
      <div className="App-header">
        <h1 style={{ textAlign: "center" }}>
          {" "}
          {"Please enter an email or phone number (digits only)"}{" "}
        </h1>
      </div>
